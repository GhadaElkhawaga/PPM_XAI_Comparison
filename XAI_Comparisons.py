...
def compare_ALE(dataset_name, method_name, bkt_size, prfx_len, feat_num, gap):
    ...
    try:
        ...
        plot_ale(ALE_xgb, features=feat_indices, targets=[target_names[0]], ax=ax, line_kw={'label': 'XGB'});
        plot_ale(ALE_logit, features=feat_indices, targets=[target_names[0]], ax=ax, line_kw={'label': 'LR'});
        plt.savefig(os.path.join(ALE_comparisons, 'ALE_comparison_%s_positiveClass.png' % (frmt_str)), dpi=300,
                    bbox_inches='tight');
        plt.clf()
    except:
        pass
    ...
    try:
        ...
        fig, ax = plt.subplots(1, len(feat_indices), figsize=(8, 4), sharey='row');
        plot_ale(ALE_xgb, features=feat_indices, targets=[target_names[1]], ax=ax, line_kw={'label': 'XGB'});
        plot_ale(ALE_logit, features=feat_indices, targets=[target_names[1]], ax=ax, line_kw={'label': 'LR'});
        plt.savefig(os.path.join(ALE_comparisons, 'ALE_comparison_%s_negativeClass.png' % (frmt_str)), dpi=300,
                    bbox_inches='tight');
        plt.clf()
    except:
        pass
