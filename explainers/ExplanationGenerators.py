...
def ALE_Computing(training_df, cls, ffeatures, dataset_name, method_name, cls_method, bkt_size, prfx_len, feat_num, target_names):
    """
    Algorithm:
    ...
    ale_pred = ALE(cls.predict_proba, feature_names=ffeatures, target_names=target_names)
    explainer_pred = ale_pred.explain(training_df.to_numpy())
    pred_exp_time = time.time() - pred_exp_start
    explainer_pred_data = os.path.join(ALE_dir, 'ALE_pred_explainer_%s_%s.pickle' % (cls_method, frmt_str))
    ...
    df_res_exp_pred_file = os.path.join(ALE_dir, 'ALE_pred_explainer_%s_%s.csv' % (cls_method, frmt_str))
    ...
    for i in files:
        ...
        try:
            ...
            for ff in corr_with_others.keys():
                if ff in ffeatures:
                    idx_ff = list(ffeatures).index(ff)
